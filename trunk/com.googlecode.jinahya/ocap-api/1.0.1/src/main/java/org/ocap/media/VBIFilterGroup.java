 ...
 * method. The host may be able to filter VBI data using separated field 
 * (specify either filed 1 or field 2 and retrieve data in only the specified 
 ...
 * mode in code (RTD or TR) and a text mode out codes (EOC, RCL, RDC, RU2, 
 * RU3 or RU4). 
 * Note that Text service data unit returned from VBIFilter doesn't contain 
 * these text mode in/out code, but contain another control code like EDM etc. 
 * </p><p>
 ...
 * Character One and Character Two makes a separated data stream in each field. 
 ...
 * rate automatically. It is not guaranteed to success filtering even if 
 * the OCAP implementation supports UNKNOWN format, since an analog waveform 
 * is not standardized. 
 ...
 * Filtering of these caption and text mode service data is not 
 * provided to display captioning text on a screen synchronizing with 
 * video and audio. It is not guaranteed that filtering performance 
 * satisfies expected delay. 
* </p><p>
 * Support of the other VBI lines are optional. And support of the ANSI/SCTE 
...
public class VBIFilterGroup implements org.davic.resources.ResourceProxy,
     ...
     *            buffer used to hold a VBI data units. 
     *
     * @throws IllegalStateException  if the total number of filters created 
     *            by this VBIFilterGroup exceeds the specified number by a 
     *            constructor. 
     *
     ...
     *            ServiceContext is in the presenting state when this method 
     *            is called. 
     *
    ...
    /**
     * Release all filters in this VBIFilterGroup and terminate the connection 
