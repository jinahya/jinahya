 ...
 * at any point, the resulting AppStateChangeEvent shall have a fromstate which is 
...
public interface AppProxy {
    ...
    /**
     * The application is not signalled in the currently selected service and
     * hence not listed in the applications database. Once an AppProxy is in this state, all 
     * attempts to cause further state transitions on that AppProxy instance shall 
     * fail with an AppStateChangeEvent from INVALID to INVALID, even if a service 
     * is selected where that application is signalled.
     */
