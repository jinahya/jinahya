 ...
 */
public class StartTask implements StateMachineTask {

    //@Override
    public void perform(int previousState, int currentState, int priority)
        throws StateMachineException {

        if (previousState == JavaTVXletSpec.PAUSED &&
            currentState == JavaTVXletSpec.STARTED) {
            System.out.println("START_TASK: startXlet");
            resource = new int[1024];
            for (int i = 0; i < resource.length; i++) {
                resource[i] = i;
            }
        } else if (currentState == JavaTVXletSpec.PAUSED) {
            if (previousState == JavaTVXletSpec.LOADED) {
                System.out.println("START_TASK: initXlet");
            } else if (previousState == JavaTVXletSpec.STARTED) {
                System.out.println("START_TASK: pauseXlet");
            }
            resource = null;
        } else if (currentState == JavaTVXletSpec.DESTROYED) {
            System.out.println("START_TASK: destroyXlet");
            resource = null;
        } else {
            // not interested
        }
