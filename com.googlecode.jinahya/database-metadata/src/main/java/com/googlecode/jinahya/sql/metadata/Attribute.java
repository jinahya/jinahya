...
public class Attribute extends ChildEntrySet<UserDataType> {
     ...
     */
    static void getAttributes(final DatabaseMetaData databaseMetaData,
                              final UserDataType userDataType)
        throws SQLException {

        if (userDataType.getMetadata().excludes.contains("getAttributes")) {
            return;
        }

        final ResultSet resultSet = databaseMetaData.getAttributes(
            userDataType.getValue("TYPE_CAT"),
            userDataType.getValue("TYPE_SCHEM"),
