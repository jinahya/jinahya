...
public class CopyControl
{
    ...
    *
    * Note (informative) OCAP Applications that have access to and are processing
    * video content should call this function at a periodic rate of no less than
    * once every minute.
    *
    * @param Service indicates the service for which the returned CCI value applies to.
    *       CCI values are passed from a CableCARD to a Host associated with a
    *       program number. The implementation SHALL use the service to identify
    *       the program number.

    *
    * @return The CCI values currently in use by the OpenCable Host Device for the
    *       indicated service.
    */
