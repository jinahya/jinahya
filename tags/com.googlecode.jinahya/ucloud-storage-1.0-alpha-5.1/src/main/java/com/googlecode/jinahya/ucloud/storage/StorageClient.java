...
//        }
    ...
    private boolean authenticate() throws IOException {
        ...
        LOGGER.log(Level.INFO, "storageUser: {0}", storageUser);
        connection.setRequestProperty("X-Storage-User", storageUser);
        LOGGER.log(Level.INFO, "storagePass: {0}", storagePass);
        ...
        try {
            ...
            if (responseCode == RESPONSE_CODE_200_OK) {
                ...
                LOGGER.log(Level.INFO, "storageUrl: {0}", storageUrl);
                authToken = connection.getHeaderField("X-Auth-Token");
                LOGGER.log(Level.INFO, "authToken: {0}", authToken);
                return true;
            }
        } finally {
...
//                                storageObjects});
    ...
    public boolean updateObject(final String containerName,
        ...
        if (!createContainer(containerName)) {
            LOGGER.log(Level.SEVERE, "failed to create container: {0}",
                       containerName);
            return false;
        }
    ...
    public boolean readObject(final String containerName,
        ...
        LOGGER.log(Level.INFO, "readObject({0}, {1}, {2})",
                   new Object[]{containerName, objectName, contentDataOutput});
