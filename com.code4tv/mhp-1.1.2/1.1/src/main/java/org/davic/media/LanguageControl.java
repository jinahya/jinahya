...
/**
 * This interface is the base interface for both audio and subtitling language control. This 
 * interface should never be implemented in a control alone, but always either as audio or subtitling 
 * language control. If a language can not be selected because the user/application is not entitled to 
 * access it, the language is reset to that before the invocation of the method.
