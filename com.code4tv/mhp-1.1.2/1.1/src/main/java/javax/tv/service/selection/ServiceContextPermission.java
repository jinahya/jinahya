 ...
 * "*". A callers "own" service contexts are those which it has created through
* ServiceContextFactory.createServiceContext(). If an Xlet would have a ServiceContext returned should it call
* ServiceContextFactory.getServiceContext(javax.tv.xlet.XletContext) then that ServiceContext is also considered
* to be one of its "own".  The string "own" means
...
public final class ServiceContextPermission extends BasicPermission {
  ...
  public boolean implies(Permission p) {
    ...
    boolean isName   = ( (getName().equals(scp.getName())) 
                            || (getName().equals("*")));
    boolean isAction = ( (getActions().equals(scp.getActions())) 
                            || (getActions().equals("*")));
